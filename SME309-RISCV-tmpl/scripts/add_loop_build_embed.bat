...
riscv-none-embed-as.exe -c src\add_loop.s -o build\add_loop.o

REM Convert to binary
riscv-none-embed-objcopy.exe -O binary build\add_loop.o build\add_loop.bin

REM Generate disassembly
riscv-none-embed-objdump.exe -d build\add_loop.o > build\add_loop.txt
